function dForm(schema, data = undefined) {
  ...
  element.classList.add("dform")
  element.innerHTML = attributesHtml
  element.schema = schema
  return element
}

function dFormData(form, validate = true) {
  const data = {}
  const checks = form.schema.map((descriptor) => {
...
function dFormShowErrors(form, errors) {
  errors.forEach((name, error) => {showError(name, error)})
}
...
function extractMultiValue(descriptor, form, validate) {
  ...
  if (control == null) {
    ...
    const values = Array(...control.selectedOptions).map(option => {return option.value})
    const result =  {
